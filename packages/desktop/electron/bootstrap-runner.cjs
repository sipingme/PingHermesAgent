...
/** Vendored install scripts shipped with PingHermesAgent (Route 2). */
function resolveVendoredInstallScript() {
  const name = installScriptName()
  const candidates = []

  if (process.env.PINGHERMESAGENT_VENDOR_INSTALL_SCRIPT) {
    candidates.push(path.resolve(process.env.PINGHERMESAGENT_VENDOR_INSTALL_SCRIPT))
  }

  // Monorepo checkout: packages/desktop/electron → repo root
  candidates.push(path.resolve(__dirname, '../../..', 'vendor/hermes-agent/scripts', name))

  // Packaged app: electron-builder extraResources → process.resourcesPath/vendor/...
  if (process.resourcesPath) {
    candidates.push(path.join(process.resourcesPath, 'vendor/hermes-agent/scripts', name))
  }
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, hermesHome, emit }) {
  // 1. Dev shortcut: sibling hermes-agent checkout (HERMES_DESKTOP_HERMES_ROOT).
  ...
  // 2. Vendored scripts bundled with PingHermesAgent (no raw.githubusercontent.com).
  const vendoredScript = resolveVendoredInstallScript()
  if (vendoredScript) {
    emit({ type: 'log', line: `[bootstrap] using vendored ${installScriptName()} at ${vendoredScript}` })
    return { path: vendoredScript, source: 'vendor', kind: installScriptKind() }
  }

  // 3. Cached copy from a prior GitHub fetch (legacy fallback).
  if (!installStamp || !installStamp.commit || !STAMP_COMMIT_RE.test(installStamp.commit)) {
    throw new Error(
      `Cannot resolve ${installScriptName()}: no local checkout, no vendored script, and no install stamp. ` +
        'Run scripts/sync-vendor-install.sh or rebuild with vendor/ present.'
    )
  }
  ...
  // 4. Last resort: download from GitHub (requires network).
