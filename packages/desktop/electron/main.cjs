...
// PingHermesAgent ships separately from the Python backend. Prefer an explicit
// checkout via HERMES_DESKTOP_HERMES_ROOT; otherwise look for a sibling
// hermes-agent directory (e.g. …/2026/hermes-agent next to …/2026/PingHermesAgent).
...
const APP_NAME = 'PingHermesAgent'
...
async function waitForHermes(baseUrl, token) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      // /api/status is public — also verify the desktop-minted session token
      // before declaring ready. Older Hermes builds ignore
      // HERMES_DASHBOARD_SESSION_TOKEN and would pass here but 401 every
      // authenticated /api call afterwards.
      await fetchJson(`${baseUrl}/api/sessions?limit=1`, token, { timeoutMs: 5_000 })
      return
    } catch (error) {
      lastError = error
...
function resetHermesConnection(reason = 'unspecified') {
  rememberLog(`[connection] reset requested (${reason})`)
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    ...
    if (hermesProcess && !hermesProcess.killed) {
      rememberLog('[connection] stopping previous Hermes backend before spawning a new one')
      hermesProcess.kill('SIGTERM')
      hermesProcess = null
    }
...
ipcMain.handle('hermes:bootstrap:reset', async () => {
  ...
  connectionPromise = null
  if (hermesProcess && !hermesProcess.killed) {
    rememberLog('[bootstrap] reset stopping live backend before renderer reload')
    hermesProcess.kill('SIGTERM')
    hermesProcess = null
  }
  bootstrapState = {
...
ipcMain.handle('hermes:bootstrap:repair', async () => {
  ...
  resetHermesConnection('bootstrap repair')
  return { ok: true }
})
...
ipcMain.handle('hermes:connection-config:apply', async (_event, payload) => {
  rememberLog(`[connection] apply requested (mode=${payload?.mode ?? 'unknown'})`)
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  resetHermesConnection('connection-config apply')
