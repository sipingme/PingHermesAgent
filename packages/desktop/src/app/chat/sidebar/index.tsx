...
export function ChatSidebar({
  ...
  const agentGroups = useMemo(
    () => orderByIds(workspaceGroupsFor(agentSessions, t('sidebar.no_workspace')), g => g.id, workspaceOrderIds),
          ...
          <SidebarSessionsSection
            ...
            emptyState={
              <div className="grid min-h-24 place-items-center rounded-lg px-2 text-center text-xs text-(--ui-text-tertiary)">
                {t('sidebar.search.no_results', { query: trimmedQuery })}
              </div>
            }
            label={t('sidebar.search.results')}
          ...
          <SidebarSessionsSection
            ...
            headerAction={
              ...
              agentSessions.length > 0 ? (
                <Button
                  aria-label={agentsGrouped ? t('sidebar.ungroup') : t('sidebar.group_by_workspace')}
                  className={cn(
                  ...
                  size="icon-xs"
                  title={agentsGrouped ? t('sidebar.ungroup') : t('sidebar.group_by_workspace')}
                  variant="ghost"
                >
...
function SidebarPinnedEmptyState() {
  ...
  return (
    <div className="flex min-h-7 items-center gap-1.5 rounded-lg pl-2 text-[0.75rem] text-(--ui-text-tertiary)">
      ...
      <span>{t('sidebar.pinned_hint')}</span>
    </div>
  )
}
...
function SidebarWorkspaceGroup({
  ...
  const { t } = useTranslation()
  const normalizedKey = group.label
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '')
  const displayLabel = t(`sidebar.workspace_name.${normalizedKey}`, group.label)
  ...
  return (
    <div className={cn('grid gap-px', dragging && 'z-10 opacity-60', className)} ref={ref} style={style} {...rest}>
      <div className="group/workspace flex min-h-6 items-center gap-1 px-2 pt-1 text-[0.6875rem] font-medium text-(--ui-text-tertiary)">
        ...
        {onNewSession && (
          <button
            aria-label={t('sidebar.new_in_group', { label: displayLabel })}
            ...
            title={t('sidebar.new_in_group', { label: displayLabel })}
            type="button"
          >
        ...
        {reorderable && (
          <span
            {...dragHandleProps}
            aria-label={t('sidebar.workspace.reorder', { label: displayLabel })}
      ...
      {open && (
        <>
          ...
          {hiddenCount > 0 && (
            <button
              aria-label={t('sidebar.workspace.show_more', { count: nextCount, label: displayLabel })}
              ...
              title={t('sidebar.workspace.show_more', { count: nextCount, label: displayLabel })}
              type="button"
            >
...
function SidebarLoadMoreRow({ loading, onClick, step }: SidebarLoadMoreRowProps) {
  const { t } = useTranslation()
  const label = loading ? t('sidebar.load_more.loading') : step > 0 ? t('sidebar.load_more.count', { count: step }) : t('sidebar.load_more.default')
