...
export function SidebarSessionRow({
  ...
  const rawAge = formatAge(session.last_active || session.started_at)
  const age = rawAge === 'now' ? t('sidebar.session.now') : rawAge
  const handleLabel = t('sidebar.session.reorder', { title })
    ...
    >
      ...
      >
        ...
        <div className="relative z-2 grid w-[1.375rem] place-items-center">
          ...
          >
            <Button
              aria-label={t('sidebar.session.actions_for', { title })}
...
function SidebarRowDot({ isWorking, className }: { isWorking: boolean; className?: string }) {
  ...
  return (
    <span
      aria-label={isWorking ? t('sidebar.session.running') : undefined}
      className={cn(
        'rounded-full',
        isWorking
